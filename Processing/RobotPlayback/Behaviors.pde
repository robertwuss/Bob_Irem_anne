...
void setupBehaviors() {
  ...
  behavior1Presence.load("incrediblePresenceRecording.txt");
...
void onInterpolatorComplete(BehaviorInterpolator interpolator){
  if(interpolator == interpolator1){
    behavior1 = interpolator1.to;
    behavior1.resume();
    println("from.lastRow",interpolator.from.lastRow,"from.data.getRowCount()",interpolator.from.data.getRowCount(),"to.lastRow",interpolator.to.lastRow,"to.data.getRowCount()",interpolator.to.data.getRowCount(),"to.lastRow",interpolator.to.lastRow,interpolator.to,behavior1);
  }
...
class Behavior {
  ...
  void resume(){
    ...
    println("isInterpolating",isInterpolating,"interpolateToFirstRowServo1.isPlaying",interpolateToFirstRowServo1.isPlaying(),"isPlaying",isPlaying);
  }

  void updateRow() {
    println("updateRow",this,currentRow);
    if (data == null) {
...
class BehaviorInterpolator {
  ...
  void onInterpolationComplete(Ani animation) {
    //println("from.lastRow",from.lastRow,"from.data.getRowCount()",from.data.getRowCount(),"to.lastRow",to.lastRow,"to.data.getRowCount()",to.data.getRowCount(),"to.lastRow",to.lastRow,to,behavior1);
    //set the target animation to the destination behaviour
    //target = to;
    // resume the behaviour
    //target.resume();
    //println("interpolation complete",this);
    onInterpolatorComplete(this);
  }

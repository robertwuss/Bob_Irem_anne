...
void setup() {
  ...
  myRemoteLocation = new NetAddress(" 192.168.2.1", 12000);
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.checkAddrPattern("/eyePosXY") == true) {
    //println ((float) theOscMessage.get(0).intValue());
    float eyePosX = theOscMessage.get(0).floatValue();
    float eyePosY = theOscMessage.get(1).floatValue();
    
    //println(eyePosY);
    
   tobii_X = map(eyePosX, 100,1000,0, 800);
   
   //float tobii_Yi = constrain(eyePosY, 1500,100);
   tobii_Y = map(eyePosY, 100, 1500, 800, 0);
   tobii_Z = tobii_Y;
...
void draw() {
  ...
  if (isTorqueOn) {
    ...
    x += (dx * .01);
    //println(x);
    pos_Xo = map(x, 0, 800, (int)1500, (int)3000);
    int xo = round(pos_Xo);
    println("XO " +xo);
